...
//!
//! Builds the same three OP routes `backends/apps/sms-gateway/src/op.rs` does,
//! duplicated here rather than imported: `backends/apps/sms-gateway` is a binary
...
// --- The OP's routes, duplicated from backends/apps/sms-gateway/src/op.rs — see this
...
/// Short relative to `backends/apps/sms-gateway`'s own `DEFAULT_KEY_REFRESH_INTERVAL`
...
/// Mirrors `backends/apps/sms-gateway/src/op.rs`'s `discovery_handler` — see this
...
/// `GatewayAuth` that `backends/apps/sms-gateway/src/main.rs` itself wires, and
...
async fn a_real_private_key_jwt_flow_mints_a_token_gatewayauth_accepts() {
    ...
    // first version of backends/apps/sms-gateway's op.rs captured JWKS once at
    ...
    // (both here and in backends/apps/sms-gateway/src/op.rs) is what this now
