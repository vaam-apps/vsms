...
//! (`backends/crates/sms-worker/src/jobs/purge_retention.rs`) against a real, fully
//! migrated Postgres.
//!
...
//!   (append-only by design; `backends/crates/sms-api/src/dlr.rs` only ever
...
//! `@@emit(created, updated)`, and `backends/crates/sms-api/src/webhooks.rs`'s
...
async fn a_purge_never_re_fires_a_webhook_to_a_registered_endpoint() {
    ...
    // production exactly: `backends/apps/sms-worker`'s `main` calls
