//! Proves #39's `drain` role (`backends/crates/sms-worker/src/drain.rs`) against a
//! real, fully migrated Postgres.
//!
//! The property worth proving isn't "subscribers work" — that's #38's own
//! `backends/crates/sms-api/tests/webhooks_live_postgres.rs`, and it already shows
...
/// on one must not leak onto the other, the same way `backends/apps/sms-gateway`
/// and `backends/apps/sms-worker` are genuinely separate processes with genuinely
...
/// `create`/`update`). See `backends/crates/sms-api/tests/webhooks_live_postgres.rs`'s
