...
fn repo_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is backends/crates/sms-api.
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../..")
        .canonicalize()
        .expect("the repo root is three levels above backends/crates/sms-api")
}
...
fn enforced_by_route_layers() -> BTreeSet<String> {
    let source = read("backends/crates/sms-api/src/router.rs");
    let mut found = BTreeSet::new();
    let mut rest = source.as_str();
...
fn granted_by_seeded_roles() -> BTreeSet<String> {
    let sql = read("backends/migrations/postgres/0002_bootstrap/up.sql");
    let start = sql
