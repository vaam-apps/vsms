...
//!
//! [`model_names_from_schema`] parses `schemas/vsms.cstack` itself for
...
//! `rotateWebhookSecret` (#41, `backends/crates/sms-api/src/procedures.rs`) and
//! #38's `Message.created`/`Message.updated` subscribers
//! (`backends/crates/sms-api/src/webhooks.rs`).
//!
...
//! moment it existed (`backends/crates/sms-worker/src/claim.rs`'s `Claimable for
...
//! it a real reader: `backends/crates/sms-worker/src/routing.rs`'s `decide`, called
...
//! (`backends/crates/sms-api/src/procedures.rs`, `sendMessage`'s own consent check
...
//! `backends/crates/sms-worker/src/jobs/grey_route_watch.rs`'s own
...
/// `backends/crates/sms-worker/tests/claim_live_postgres.rs`'s doc (#102).
...
/// `run(&system_context())` call site in `backends/crates/sms-api`,
/// `backends/crates/sms-worker`, and `backends/crates/sms-auth`:
///
/// - `App`, `AppClient` — `Procedures::resolve_app` (`backends/crates/sms-api/src/procedures.rs`)
///   and `GatewayAuth`'s own `client_id` lookup (`backends/crates/sms-api/src/auth.rs`).
/// - `OauthClient` — `SmsClientStore` (`backends/crates/sms-auth/src/lib.rs`).
/// - `OauthSigningKey` — `sms_auth::op` load/rotate (`backends/crates/sms-auth/src/op.rs`).
/// - `ClientAssertion` — `SmsClientAssertionStore` (`backends/crates/sms-auth/src/lib.rs`).
/// - `SenderId`, `SenderIdRegistration` — `Procedures::resolve_sender_id`.
/// - `Provider` — `backends/crates/sms-worker/src/routing.rs`'s `decide` (since #62;
///   formerly `cheapest_active_provider`, which this replaced) and
///   `Procedures::estimate_cost`.
/// - `Route` — `backends/crates/sms-worker/src/routing.rs`'s `decide` (#62), the
...
/// - `Message` — the claim loop's own `candidates()` (`backends/crates/sms-worker/src/claim.rs`)
///   and `dlr::ingest_one`.
...
///   subscribers (`backends/crates/sms-api/src/webhooks.rs`,
...
///   WebhookAttempt::candidates`, `backends/crates/sms-worker/src/claim.rs`, #40)
...
///   (`backends/crates/sms-api/src/auth.rs`), both necessarily running before any
...
///   own `check_overdue_validations` (`backends/crates/sms-worker/src/jobs/grey_route_watch.rs`)
...
const SYSTEM_READABLE_MODELS: &[&str] = &[
    ...
    "UserCredential",
    // #68: `backends/crates/sms-worker/src/jobs/anchor_audit.rs`'s own `latest_anchor`/
...
/// Parses `schemas/vsms.cstack` for every top-level `model Foo {` line —
...
fn model_names_from_schema() -> Vec<String> {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../schemas/vsms.cstack");
...
/// #62: `backends/crates/sms-worker/src/routing.rs`'s `decide` reads `Route` under
...
/// (`backends/crates/sms-worker/src/claim.rs`) reads due `WebhookAttempt` rows under
/// `sys` — the first internal system-context reader this model has ever
...
/// (`backends/crates/sms-worker/src/jobs/anchor_audit.rs`) read this model under
...
/// `@@allow("read", ...)` in `schemas/vsms.cstack`, this test failed on
