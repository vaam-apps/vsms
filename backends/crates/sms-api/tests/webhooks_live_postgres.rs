...
//! (`backends/crates/sms-api/src/webhooks.rs`) against a real, fully migrated
...
//! property worth proving is the one `backends/apps/sms-gateway` and
//! `backends/apps/sms-worker` actually rely on: that a plain mutation on `Message`,
...
/// read directly in `backends/crates/sms-worker/src/jobs/reap_outbox.rs`'s own module
...
/// writer in this codebase (`backends/apps/sms-gateway`'s `serve`, `backends/apps/sms-worker`'s
...
/// see `backends/apps/sms-gateway/tests/webhook_outbox_kill_mid_drain_live.rs`. This
