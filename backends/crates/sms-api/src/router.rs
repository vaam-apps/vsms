...
/// (`backends/crates/sms-api/src/auth.rs`) constructs it exactly once, inside
...
/// `frontends/packages/gateway/src/senders.ts`/`webhooks.ts`'s own module docs for the
...
/// live HTTP test suites in `backends/crates/sms-auth/tests/`) shares one source of
...
/// asserted: only `backends/crates/sms-auth/tests/{rbac_layer2_live_postgres,
/// oidc_flow_live}.rs` and `backends/apps/sms-gateway/tests/*_live_postgres.rs` ever
...
/// a Caddy edge in front of it (`backends/apps/sms-gateway/src/main.rs`'s own
...
/// webhook, or `/healthz`, all of which `backends/apps/sms-gateway/src/main.rs`
...
/// because `backends/apps/sms-gateway/src/main.rs` serves this router through
