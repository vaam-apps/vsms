...
//!    see `frontends/apps/admin/lib/oidc.test.ts` for that half's guard-failure proof).
//!
...
//! `backends/crates/sms-auth/tests/login_live_postgres.rs` for its own live proof)
