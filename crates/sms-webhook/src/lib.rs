...
//!
//! `crates/sms-worker`'s `hooks` role (#40) is the intended sender-side
//! caller of [`sign_header`]; `crates/sms-api`'s `rotate_webhook_secret`
...
//! comment in `crates/sms-api/src/procedures.rs` for the current, narrower
...
/// Matches `crates/sms-api/src/pepper.rs`'s `MIN_PEPPER_BYTES`: 256 bits,
...
/// `crates/sms-api/src/pepper.rs`'s `hmac_sha256_hex`.
#[must_use]
...
/// (`crates/sms-api/src/procedures.rs`, #41) is the intended caller.
///
/// # Panics
///
