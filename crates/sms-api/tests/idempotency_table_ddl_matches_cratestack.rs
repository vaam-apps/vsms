//! #153: `schema/migrations/postgres/0003_idempotency_table/up.sql`
//! hand-copies `cratestack`'s own `IDEMPOTENCY_TABLE_DDL` constant —
//! `crates/sms-api/src/router.rs`'s `router()` deliberately never calls
//! `SqlxIdempotencyStore::ensure_schema()` itself (see that function's own
//! doc for why `app/sms-migrate`/`ci/apply-migrations.sh` own this table
...
/// `src/lib.rs` for `../../schema/schema.cstack` — three directories up
/// from this crate (`crates/sms-api/`) to the repository root, then into
/// `schema/migrations/postgres/0003_idempotency_table/up.sql`.
const IDEMPOTENCY_TABLE_SQL: &str =
    include_str!("../../../schema/migrations/postgres/0003_idempotency_table/up.sql");
...
fn idempotency_table_ddl_carries_cratestacks_own_ddl_verbatim() {
    ...
    assert!(
        ...
        "schema/migrations/postgres/0003_idempotency_table/up.sql no longer matches \
