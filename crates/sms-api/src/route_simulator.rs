...
//!
//! `sms_routing::select_route` (`crates/sms-routing`, #62) already computes
...
//! needs (identical shape to `crates/sms-worker/src/routing.rs::decide`) or
...
//!
//! `crates/sms-worker/src/routing.rs` already does exactly this fetch +
...
//! `sms-worker.workspace = true` sitting in `crates/sms-api/Cargo.toml`'s
...
/// `crates/sms-worker/src/routing.rs::decide` uses — `priority` desc then
