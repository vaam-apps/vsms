...
//!
//! `health.rs`'s own module doc (`app/sms-gateway/src/health.rs`) already
...
//! to its own listener on its own address (`app/sms-gateway/src/main.rs`'s
...
//! port today. `app/sms-worker`'s own `main.rs` binds [`router`] to its own
//! second listener too (`--metrics-listen`, default `127.0.0.1:9091`) —
//! reused directly rather than duplicated, since `app/sms-worker` already
