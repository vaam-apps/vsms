...
//! the process that *takes* a lock (`crates/sms-worker/src/lease.rs`) and
...
//!
//! `crates/sms-worker/src/lease.rs::advisory_lock_key` sources its values
//! from [`lock_key_for_role`] rather than keeping its own copy, and
//! `crates/sms-worker/src/lease.rs`'s own test module cross-checks every
...
/// outside the six §7.1 roles. `crates/sms-worker/src/lease.rs`'s
