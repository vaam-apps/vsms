...
//!
//! `app/sms-gateway/src/dlr.rs` owns the real `POST /dlr/{providerKey}`
//! route, but `app/sms-gateway` is a binary crate with no `lib.rs`, so its
...
/// why this duplicates (rather than imports) `app/sms-gateway/src/dlr.rs`'s
/// own handler.
#[derive(Clone)]
...
/// same `POST /dlr/{providerKey}` shape `app/sms-gateway/src/dlr.rs` mounts
