...
//! rather than panicking on its first request. `app/sms-gateway/src/
//! main.rs`'s `install_default_crypto_provider` (and
//! `app/sms-worker/src/main.rs`'s identical copy) make that call the very
...
//! `ORANGE_CM_BASE_URL` defaults to in `app/sms-gateway/src/main.rs` and
//! `app/sms-worker/src/main.rs`, so this test's request is a real instance
