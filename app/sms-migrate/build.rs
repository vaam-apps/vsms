...
//! every subdirectory of `schema/migrations/postgres` that contains an
...
//! `cratestack migrate diff --out-dir schema/migrations/postgres --name
...
//! the SQL gets embedded. `app/sms-migrate/Dockerfile`'s runtime image
...
fn main() {
    ...
    let migrations_root = Path::new(&manifest_dir).join("../../schema/migrations/postgres");
    ...
    // `crates/sms-test-support`'s own `migration_dirs()`).
    names.sort();
