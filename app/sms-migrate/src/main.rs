...
//! schema/migrations/postgres --name <name>` is what produces it, and
...
//!
//! [`build.rs`](build.rs) walks `schema/migrations/postgres` at compile
...
//! `crates/sms-test-support`'s own `migration_dirs()` already read. A
...
//! fourth entry has relocated to `schema/migrations/postgres/
...
/// `schema/migrations/postgres` that contains an `up.sql`, in lexical
