...
// seam as `client.ts` (see its module doc for why: T3/`frontends/packages/sms-client`
 ...
 * in a genuine response body (`frontends/apps/admin/app/messages/[id]/timeline.ts`'s own
 ...
 * `frontends/packages/gateway/src/json.ts` is now the one place this package converts
...
// `schemas/vsms.cstack`'s own comment on the procedure declaration and
// `backends/crates/sms-api/src/procedures.rs::message_receipts`'s doc for why:
...
// `frontends/apps/admin/app/messages/[id]/timeline.ts`'s own module doc for why: a
...
// `next_state`'s own outcome-to-transition mapping (`backends/crates/sms-api/src/
