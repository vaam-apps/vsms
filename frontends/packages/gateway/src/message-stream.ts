...
// `frontends/packages/hooks/src/provider.tsx`'s own module doc already commits to
// this — the browser never gets a real subscription transport
// (`httpSubscriptionLink`/SSE), only `httpBatchStreamLink` — so
// `frontends/packages/api/src/routers/messages.ts`'s `onStateChange` wraps this
