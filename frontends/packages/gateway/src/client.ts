...
// generate `frontends/packages/sms-client` from `schemas/vsms.cstack` once an
...
// prefers. `frontends/packages/api/src/routers/compose.ts` only ever imports
...
// (`schemas/vsms.cstack`'s `procedure previewMessage` / `mutation
// procedure sendMessage`).
//
// Types below are hand-transcribed from `schemas/vsms.cstack`'s
...
// `frontends/packages/api/src/routers/compose.ts`) is typed the second way. Writing
 ...
 * human's own token (#211).** `backends/crates/sms-api/src/procedures.rs::
