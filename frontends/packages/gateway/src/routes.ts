...
// bug — `frontends/packages/gateway/src/json.ts` now does this once, for every field
 ...
 * for a reproducible weighted draw (`backends/crates/sms-routing/src/engine.rs`'s
 ...
 * `{ name: string | undefined }` (from `frontends/packages/api/src/routers/routes.ts`'s
