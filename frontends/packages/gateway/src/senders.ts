...
// doc for why: T3/`frontends/packages/sms-client` is blocked on an upstream cratestack
// release).
//
...
// here since nothing in `backends/crates/sms-api` queries or branches on
 ...
 * every response this package parses, by `frontends/packages/gateway/src/json.ts` —
