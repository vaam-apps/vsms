...
import { cn, isQuietHue } from "@vaam-apps/ui";
...
const OUTCOME_CLASSES: Record<OutcomeKind, string> = {
  ...
  // unconditionally rendered "eligible" as an invisible box instead of a
  // pill. Draw the achromatic surface every quiet hue uses instead, and
  // keep only the hue's own foreground colour.
  eligible: isQuietHue("success")
    ? cn("border-edge", "bg-surface-2", "text-state-success-fg")
    : cn("border-state-success-border", "bg-state-success-bg", "text-state-success-fg"),
};

export function OutcomePill({ outcome }: { outcome: OutcomeKind }) {
  return (
    <span className={cn("rounded-sm border px-1.5 py-0.5 text-caption", OUTCOME_CLASSES[outcome])}>
