...
// on this screen repeats (`AttemptQuickDetailBody`, `EndpointQuickDetailBody`).
// Not created as a shared `@vsms/ui` primitive — the identical shape repeats
// independently in `routes`/`sender-ids`/`providers` too, each factored the
// same way in its own `components/detail-row.tsx`; see that file's own doc
// comment (`routes/components/detail-row.tsx`) for why it stays route-local.
