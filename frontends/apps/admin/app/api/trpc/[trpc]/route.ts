...
// `frontends/packages/gateway/src/request-credential.ts`'s own module doc for why
// that seam exists and why it lives here rather than threaded through
// every tRPC procedure/router individually. `frontends/apps/admin/middleware.ts` has
...
// router (`frontends/packages/api/src/routers/*.ts`) and every `@vsms/gateway`
