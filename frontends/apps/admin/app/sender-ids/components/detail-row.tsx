...
// Route-local (R6): the labelled `dt`/`dd` row `SenderQuickDetailBody`'s
// own summary `dl` repeats. Not created as a shared `@vsms/ui` primitive —
// the identical shape repeats independently in `routes`/`webhooks`/
// `providers` too, each factored the same way in its own
// `components/detail-row.tsx`; see that file's own doc comment
