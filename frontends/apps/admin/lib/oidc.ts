 ...
 * import at module scope. Deliberately so: `frontends/apps/admin/middleware.ts` (Edge
 ...
 * the route handlers under `frontends/apps/admin/app/api/auth/` (Node runtime) need the
 ...
 * see `frontends/packages/gateway/src/token.ts`) is built specifically to run
 ...
 *   `frontends/apps/admin/middleware.ts`'s own doc).
 *
...
/**
 * Constant-time equality over two strings — `frontends/apps/admin/middleware.ts`'s
 ...
 * `backends/apps/sms-gateway/src/login.rs`'s own doc).
 */
