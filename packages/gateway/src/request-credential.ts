...
// call
//
// `packages/api/src/context.ts`'s tRPC `Context` already carries
// `gateway: typeof gateway` — a reference to this whole module, not a
// per-request-bound instance — and every router (`packages/api/src/
...
//    boundary (`admin/app/api/trpc/[trpc]/route.ts`'s `handler`), read
...
//   (`admin/lib/oidc.ts`), minted by `sms-gateway`'s real
...
//
// `admin/middleware.ts` already refreshes a human session's access token
...
// has nothing further to do for expiry. See `admin/lib/oidc.ts` and
// `admin/middleware.ts`'s own docs for the mechanism.
 ...
 * `admin/app/api/trpc/[trpc]/route.ts`'s `handler` is the one call site
  ...
  if (credential === undefined) {
    throw new Error(
      "resolveUpstreamAccessToken() called outside a request-credential scope. " +
        ...
        "own machine credential — it presents. See packages/gateway/src/" +
 ...
 * own session cookie, already refreshed by `admin/middleware.ts` before the
