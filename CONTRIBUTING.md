...
A change lands in three places, or it has not landed: the code, its docs (the
module's `.md` sidecar, `docs/architecture.md` when the spec moves, the
runbooks), and the agent skills in
[vaam-apps/vsms-skills](https://github.com/vaam-apps/vsms-skills). Those skills
brief the agents that work on vsms, and an agent does not merely trust a stale
skill: it acts on it, in every session that loads it.

- **Which skill a change touches:** the routing table in vsms-skills,
  [`skills/vsms-docs-skills/references/skills-parity.md`](https://github.com/vaam-apps/vsms-skills/blob/main/skills/vsms-docs-skills/references/skills-parity.md).
  For example, `frontends/**` goes to `vsms-console`, and a trap you only found
  by running it goes to `vsms-troubleshooting`.
- **The gate:** vsms-skills' `node tools/verify-coverage.mjs <path-to-vsms>`,
  run by that repository's CI on every push and daily against this
  repository's `main`. It fails in both directions: a documentation page no
  skill covers, and a path a skill claims that no longer exists here. Green
  means every page is *claimed*. It cannot read prose, so it never means the
  claiming skill says anything *true*. That part is the reviewer's.
- **In the pull request:** open the vsms-skills PR alongside this one and link
  the two. State what happened to each companion in the template's
  "Companions" section: a link, or `n/a — <reason>`. A bare "n/a" is an
...
- State the companions, docs and vsms-skills, in the template's "Companions" section (see [Docs↔skills parity](#docsskills-parity)).
