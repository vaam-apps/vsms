...
(``docs/runbooks/36-handset-gate.adoc``'s own precedent: some things need a
...
route silently rewrites the sender ID _after_ this system has already
...
requirement ``docs/runbooks/36-handset-gate.adoc``'s own Test 1 has.
...
. *Send a real message through the specific route being validated.*
...
(``docs/runbooks/36-handset-gate.adoc``'s own trigger) sends a real message
...
actually resolve to, _before_ you spend a real SMS confirming it.
. *Watch the handset.* Record exactly what the sender ID field shows --
...
. *Record the result*, whether it passed or failed:
+
[,bash]
----
...
. *On a failure*, treat the route as suspect _now_, not after a
