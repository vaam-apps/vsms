...
Everything runs as a container, built from this checkout's own source -- `compose.dev.yaml` (see that file's own header for the full design, including why it's a third, separate compose file alongside `compose.yml` and `compose.demo.yaml` rather than a reuse of either). From a cold start this takes a few minutes (it builds every image). It brings up, in order: a scratch Postgres, both migrations, an OP signing key, an `App` + `Provider` + `Route` + `SenderId` + `WebhookEndpoint`, a machine client, the `sms-console` OIDC client, a human operator account, `sms-fake-orange`, `sms-gateway`, `sms-worker` (`dispatch,scheduler,jobs,drain,hooks`), the admin console, and `demo-app` -- see "The demo app" below.
...
`just demo` ends by waiting for `demo-app` (link:../../examples/node/demo-app[`examples/node/demo-app`] -- own README there) to run to completion and printing its log. It is not another fixture-seeding step: it authenticates over `private_key_jwt` with `@vymalo/vsms-node` (the same published SDK an integrator would use), sends one real `otp`-class message, polls it to a terminal state, and verifies the `hooks` worker role's own signed webhook for it against a receiver it runs itself -- the one thing nothing else in this stack proves end to end. Expect a timeline like:

[,text]
----
[demo-app] webhook receiver listening on :9000 (POST /webhooks)
[demo-app] issuer=http://sms-gateway:8080 clientId=appc_... to=+237677123456 senderId=VSMS
[demo-app] accepted messageId=c... state=accepted encoding=gsm7 segments=1 operator=mtn estimatedCostXaf=15
[demo-app] state=routed   (+1002ms)
[demo-app] state=submitted  (+2004ms)
[webhook] VERIFIED  type=message.accepted messageId=c... state=accepted
[demo-app] state=delivered  (+3512ms)
[webhook] VERIFIED  type=message.delivered messageId=c... state=delivered
...
=== webhook events received ===
   +512ms  message.accepted messageId=c... — verified
  +3890ms  message.delivered messageId=c... — verified

[demo-app] SUCCESS: messageId=c... reached delivered with 2 verified webhook(s) of 2 received
----
