...
- **`vsms-backup restore`** — the reusable restore primitive. Pulls a
  ...
  `--clean` drops and recreates every object *inside* the target
...
- **`vsms-backup restore-drill`** — the actual gate. Seeds a recognisable
  marker row, records an *exact* row count for every table (a real
  ...
  *mechanism* is broken, not that the drill tool itself failed for some
...
`app/sms-migrate` binary (not `./ci/apply-migrations.sh`, deleted in the
...
the drill, and `BACKUP_RCLONE_REMOTE` pointed at a *named* rclone remote
...
| Step | Result |
|---|---|
| `providers` row inserted before the drill | present |
| Row counts across every `public` table + `schema_migrations` + the marker table, before vs after | **identical**, table-by-table exact `count(*)` — e.g. `operator_prefix_rules=14`, `message_state_transitions=25`, `job_state_transitions=9`, `providers=1` |
...
| `vsms-backup restore-drill`'s own exit code | `0`, printing `RESTORE DRILL PASSED` |
| Pepper fingerprint check, matching pepper | `pepper fingerprint matches` |
| Pepper fingerprint check, deliberately wrong pepper (negative test) | `WARNING — SMS_HASH_PEPPER does not match...`, restore still completed (warn, not block — as designed) |
...
That is a *stronger* result than the row-count-mismatch path was designed
