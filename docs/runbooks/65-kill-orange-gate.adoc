...
Per https://github.com/vymalo/vsms/issues/65[#65] and §12 of `docs/architecture.md`
...
. MTN traffic keeps flowing.
. Orange-destined traffic fails over to MTN and still reaches `submitted`.
. Nothing double-sends.
. The circuit breaker reopens once Orange is reachable again.

Same split as `docs/runbooks/36-handset-gate.adoc` (M2's own gate, and the model
...
``docs/runbooks/36-handset-gate.adoc``'s own prerequisites describe -- this
...
. *Revoke or rotate the Orange `client_id`/`client_secret`* at the
...
. *A firewall rule or NAT egress block* against Orange's API host from
...
_does_ open the circuit breaker (`ProviderError::Unavailable` →
`OpenCircuitAndTryNextRoute`).
. *Point `--orange-base-url` at an address nothing listens on*, restart
...
to mean if reversing the _same_ mechanism is what's checked in the recovery
step.

== Running the gate

. *Send a burst of MTN-destined traffic* (`operator: mtn` in the send
...
. *Send a burst of Orange-destined traffic* and confirm it reaches
`delivered` normally too -- the pre-outage baseline for clause 2.
. *Interrupt Orange* (see above), noting the wall-clock time.
. *Immediately send a fresh burst of MTN-destined traffic.* Watch it
...
. *Immediately send a fresh burst of Orange-destined traffic.* Watch each
...
. *Watch the Providers screen (#54).* After enough consecutive failures
(§6.3: five) against the interrupted provider, its circuit should show
as open, with the remaining wait time visible.
. *Check for duplicates the way the automated suite does -- against
...
. *Restore Orange connectivity* -- reverse whichever mechanism was used in
step 3.
. *Send a fresh burst of Orange-destined traffic* once the circuit's
...
consult), that is an accepted gap in _this_ runbook, not a failed gate -- say
so plainly in the record of the run, the same way `docs/runbooks/36-handset-
gate.adoc` names what its own dry run could and couldn't prove.
...
could have used) becoming _visible_ in practice, not just theoretical.
...
`OPEN_QUESTIONS.md` §2.3, about a crash _mid_-submit, not this gate's own
...
downstream. That is a _stronger_ result than failing deep in the
