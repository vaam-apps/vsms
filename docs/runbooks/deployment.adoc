...
`vsms-demo-seed` (`ghcr.io/vaam-apps/vsms/demo`, a separate binary/image --
...
[,bash]
----
git clone https://github.com/vaam-apps/vsms.git
cd vsms/deploy
cp .env.example .env
chmod 600 .env
...
*R4 (`CONTRIBUTING.md`: "the admin console is optional, the backend
must run without it") -- review round 1, blocker 1: the command below
is for a deployment bringing up the console.* For a genuinely
backend-only deployment, omit `--console-redirect-uri`/`--owner-email`/
`--owner-display-name` entirely -- `bootstrap` still creates the
signing key and the `orange_cm` `Provider`/`Route` (everything
`sms-gateway`/`sms-worker` themselves need to start), and explicitly
skips the console-client and owner-account steps rather than either
attempting them with nothing to configure them from or silently doing
nothing. Passing `--owner-email` without `--console-redirect-uri` is
refused with a named error, not silently accepted -- an operator
account with no console `redirect_uri` registered has nothing to sign
into.
...
[,bash]
----
mkdir -p secrets && chmod 711 secrets
----

*Production-readiness audit B2 (review round 1, blocker 2): the private
key now gets provisioned by two dedicated one-shot compose services,*
`provision-console-client` and `secrets-fix-perms`, *not a runbook
incantation an operator has to get exactly right by hand.* Two real bugs
in the previous revision of this step were found live, by actually
running it end to end, not by inspection:

* `sms-gateway` itself runs as the distroless image's own uid `65532`; a
  freshly `mkdir -p secrets && chmod 700 secrets`'d host directory (this
  file's own earlier revision) is owned by the operator's own host user,
  which `65532` cannot write into at all.
* `chmod 0644` on the resulting private key -- this file's own earlier
  fix -- is *wrong on its own terms*, not just unreachable: world-readable
  is not an acceptable mode for a private key, regardless of who could
  actually run the command to set it.

`provision-console-client` runs as `user: '0:0'` (root bypasses
host-side Unix permission checks on a bind mount entirely, confirmed
live), so it can write into `secrets/` regardless of who owns it.
`secrets-fix-perms` then `chown`s the key to admin's own runtime uid
(`10001` -- `frontends/apps/admin/Dockerfile`'s `useradd --uid 10001`)
and locks it to `0400` -- readable by that uid alone, not `0644`.
`chmod 711` on the directory itself (not `700`) matters just as much:
verified live, a directory with no execute ("search") bit for "other"
blocks *any* other uid from even traversing into it, regardless of what
mode the file inside has -- `711` allows traversal without allowing
directory listing.

`SMS_CONSOLE_APP_ID` (the App id from step 5's own `create-app` above)
has to be set for the first command below to succeed -- either export it
inline, or add it to `.env`:

[,bash]
----
SMS_CONSOLE_APP_ID=<the App id> \
  docker compose --env-file .env --profile console up secrets-fix-perms
----

*Run this exactly once* -- `secrets-fix-perms` depends on
`provision-console-client`, and a *second* `up secrets-fix-perms`
re-attempts `provision-console-client` too, which refuses to overwrite
an already-existing `--key-out` (a real, intentional safety property --
see `ProvisionClientResult::privateKeyPem`'s own doc: it's returned
exactly once and never stored anywhere else, #23/#111) and fails the
whole chain. To redo this step, remove
`secrets/console-private-key.pem`/`secrets/console-client-id` by hand
first.

Set `SMS_CONSOLE_CLIENT_ID` in `.env` to `$(cat secrets/console-client-id)`
(or the `provisioned client: <id>` line in `docker compose logs
provision-console-client`).

*Proven live, not just described:* a `busybox` container run as uid
`10001` (through the same `configs:` mechanism `admin` itself uses) can
read the resulting key; the same container run as `65532` (sms-gateway's
own uid) or any other uid gets `Permission denied` on both the file and
...
== Enabling MTN delivery receipts (MADAPI)

Only if this deployment sends through MTN. Orange needs nothing here --
its DR endpoint is whitelisted by a manual support ticket, covered in
xref:36-handset-gate.adoc[36-handset-gate.adoc].

*MTN delivery receipts do not arrive unless you register an endpoint
first.* MADAPI requires two independent things: `requestDeliveryReceipt:
true` on every outbound message, which `sms-provider-mtn` always sends,
and a `deliveryReportUrl` registered against your approved
`serviceCode`. Skip the registration and messages submit successfully,
reach `submitted`, and then sit there permanently with no receipt ever
arriving -- which looks exactly like a broken route and is why this
section exists rather than a line in a table.

Unlike Orange's whitelisting, this is a real API call, so run it once per
approved short code:

[,bash]
----
docker compose run --rm sms-gateway mtn-subscribe-dlr \
  --delivery-report-url "https://$SMS_GATEWAY_DOMAIN/dlr/mtn_cm" \
  --target-system vsms
----

`MTN_CLIENT_ID`, `MTN_CLIENT_SECRET`, `MTN_SERVICE_CODE` and
`MTN_BASE_URL` come from the environment (see `deploy/.env.example`), so
they are not repeated on the command line -- the secret in particular
should never land in shell history.

Note the path segment is `mtn_cm`, matching `SmsProvider::key` and what
``sms-gateway``'s `+/dlr/{providerKey}+` handler matches against. A
subscription registered against any other path will be accepted by MTN
and then silently 404 on every receipt.

`run --rm`, not `exec`: this command needs no database and no running
gateway, and the gateway may legitimately not be up yet.
...
to its own crate/image, `vsms-demo-seed`, `ghcr.io/vaam-apps/vsms/demo`,
...
----
cd deploy
export VSMS_IMAGE_TAG=v0.3.1   # or the tag you're upgrading to
docker compose --env-file .env pull
docker compose --env-file .env up -d
----

`VSMS_IMAGE_OWNER` (default `vaam-apps`) is the second half of that
reference -- `release.yml` publishes to
`ghcr.io/${{ github.repository_owner }}/...` unconditionally, and this
repository's own GitHub owner has moved twice: `v0.2.1` and earlier
published under `ghcr.io/vymalo/...`, `v0.3.0` alone under
`ghcr.io/vaam-store/...`, and `v0.3.1` on lands under `ghcr.io/vaam-apps/...`,
this file's own default now. Pulling an older release means setting BOTH
knobs together, never one without the other:

[,bash]
----
export VSMS_IMAGE_OWNER=vymalo
export VSMS_IMAGE_TAG=v0.2.1
----
...
`SMS_HASH_PEPPER`, `ORANGE_CM_CLIENT_SECRET`, `MTN_CLIENT_SECRET` when
MTN is configured, `SMS_CONSOLE_SESSION_SECRET`, the admin console's
private key PEM) is referenced by name from an `existingSecrets.*` value; nothing under `deploy/charts/vsms/templates`
