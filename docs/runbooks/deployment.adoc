...
`vsms-demo-seed` (`ghcr.io/vaam-store/vsms/demo`, a separate binary/image --
...
to its own crate/image, `vsms-demo-seed`, `ghcr.io/vaam-store/vsms/demo`,
...
----
cd deploy
export VSMS_IMAGE_TAG=v0.3.0   # or the tag you're upgrading to
docker compose --env-file .env pull
docker compose --env-file .env up -d
----

`VSMS_IMAGE_OWNER` (default `vaam-store`) is the second half of that
reference -- `release.yml` publishes to
`ghcr.io/${{ github.repository_owner }}/...` unconditionally, and this
repository's own GitHub owner is `vaam-store` as of `v0.3.0`. Every image
through `v0.2.1` published under the prior owner, `ghcr.io/vymalo/...` --
pulling one of those older releases means setting BOTH together, never
