...
approved `SenderId` + `WebhookEndpoint`, seeded by the dedicated
`vsms-demo-seed` binary/image -- see
``backends/apps/vsms-demo-seed/src/main.rs``'s own doc for why this step exists
...
(`dispatch,scheduler,jobs,drain,hooks`), `sms-fake-orange`, `admin`, and
`demo-app` -- see "The demo app" below. Every one of those seed/provision
steps is chained onto the previous one's completion, not just onto
`migrate` -- found live, twice: two of them touching a _genuinely fresh_
database's catalog concurrently can race Postgres's own `pg_type` unique
index (``compose.demo.yaml``'s own comment on `seed-dispatch` has the full
mechanism), so this file trades a few seconds of extra startup time for
never hitting that race instead of hoping not to.

NOTE: `seed-demo-app`'s `--webhook-secret-out` flag, and `demo-app` itself,
need a `VSMS_DEMO_IMAGE_TAG`/`VSMS_IMAGE_TAG` that includes them --
``compose.demo.yaml``'s own header comment already documents this exact
kind of gap for `provision-client`'s newer flags, and this is the same
shape: until a tag publishes them, override both to a tag that does.
...
[,bash]
----
docker compose -f compose.demo.yaml --profile console logs demo-app
----
