...
`POST /dlr/orange_cm` route, whitelisted with Orange support ahead of time
-- confirmed against Orange's own documentation (§4 "About SMS Delivery
Receipt", https://developer.orange.com/apis/sms/getting-started): the DR
endpoint "must be whitelisted on the Orange SMS API server before being
used; to do this, please fill in this web form." This is a real
prerequisite, not an optional one -- there is no per-request `notifyURL`
field on the submit request any more (`OrangeCmConfig` carries no such
knob; see `sms-provider-orange-cm`'s own `lib.rs` module doc), so the
whitelisted, pre-registered endpoint is the *only* way a DLR can ever
reach this deployment. If the first attempt at test 1 sends the SMS but
never reaches `delivered` in the database, the whitelisted URL is the
first thing to re-check with Orange support (see "If it doesn't reach
``delivered``" below).
...
-- check the HTTPS front-end is actually up and the URL is exactly what
Orange support whitelisted via the web form (§4 "About SMS Delivery
Receipt", https://developer.orange.com/apis/sms/getting-started). There is
no per-request override to fall back on any more: `OrangeCmConfig` carries
no `notifyURL`/`dlr_notify_url` field at all, since Orange's own docs
describe the DR endpoint as pre-registered and account-level, not
something a submit request can specify -- the whitelisted URL genuinely
is the only path a DLR can take.
