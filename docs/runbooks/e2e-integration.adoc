...
https://github.com/vymalo/vsms/issues/144[#144] proved a message sent from the admin
console's own composer reaches `delivered`, and
https://github.com/vymalo/vsms/issues/149[#149] proved a third-party Rust backend can
...
`openssl dgst -sign`, `curl`, `jq`) is gone, replaced by link:../../ci/e2e-integration[`ci/e2e-integration`],
...
self-contained and safely rerunnable -- it brings up `compose.dev.yaml` fresh (`just
demo-up`, which itself wipes and recreates every named volume) before every run, so
running it twice in a row with no manual cleanup in between is the normal way to use it,
not a special "cold reset" mode. It exits non-zero at the first broken link, naming the
step.

`demo-up`, not `demo`: this recipe brings the stack up with `--profile console --profile
demo` (so `demo-app` and its own `WebhookEndpoint` genuinely exist, the same shape a real
run has) but never touches `demo-app` itself and doesn't wait on it -- `demo` (`just
demo`'s own recipe) additionally waits for `demo-app` to finish and propagates its exit
code, which this runbook's own assertion has nothing to do with. Splitting them was a
direct fix for a real bug: an earlier revision had `e2e-integration` depend on `demo`
itself, which meant a `demo-app` failure -- real or a flaky run, unrelated to what this
runbook checks -- could fail `just e2e-integration` too, for a reason this runbook's own
assertion never touched.

== What it does

. *Brings the stack up* via `just demo-up` -- every service as a container built from this
checkout's own source (`compose.dev.yaml`), including a scratch Postgres,
`sms-gateway`, `sms-worker` (`dispatch,scheduler,jobs,drain,hooks`), `sms-fake-orange`, the
admin console, and `demo-app` (started, per the `--profile demo` this recipe now passes,
but never waited on or asserted against here -- see this file's own opening section), plus
one `App` (`vsms-demo`) and a "demo console" `AppClient`.
...
. *Sends as the integrator, over real HTTP*, via `ci/e2e-integration` -- its own
...
_the_ message, not _a_ message (#160's own acceptance criterion 5). Runs _inside_
...
audience has to match that exact issuer -- a host process can only ever _connect_ to the
...
. *Builds a second, independent client* -- the console's own credential, extracted from
...
. *Polls `+GET /messages/{id}+` as the console*, once a second, until that exact id
...
this on screen, verbatim: _"Scoped to this app only -- the console's own service-account
token can only read the one app it belongs to, so there is nothing to switch to. This is
not a filter and not a bug."_
...
"provision the two clients under _different_ ``App``s and confirm an operator token _can_
...
readiness*. xref:36-handset-gate.adoc[`36-handset-gate.adoc`] -- a real Orange account and a
...
*Run 2* (immediately after Run 1, ``just e2e-integration``'s own `just demo-up` prerequisite
...
-- xref:36-handset-gate.adoc[`36-handset-gate.adoc`]'s own scope, unaffected by this work.
...
* Concurrent runs of `just e2e-integration` on one machine -- like `just demo`/`just demo-up` themselves, the
