...
https://github.com/vymalo/vsms/issues/70[#70]/https://github.com/vymalo/vsms/issues/71[#71],
epic https://github.com/vymalo/vsms/issues/66[#66], plus two more from
https://github.com/vymalo/vsms/issues/64[#64] (grey-route detection,
epic https://github.com/vymalo/vsms/issues/60[#60]). §9.1 of
link:../architecture.md#91-observability[the design doc] is the spec #70/#71
implement; link:../../backends/crates/sms-metrics/src/lib.rs[`backends/crates/sms-metrics/src/lib.rs`]'s
...
. *`sms-gateway`*, inside `sendMessage` (`backends/crates/sms-api/src/
...
for _this one HTTP request_ together, whether that request came in with
...
one.
. *`sms-worker`*, inside `dispatch::submit_one`
...
. *`sms-gateway`* again, inside DLR ingestion (`backends/crates/sms-api/src/
...
and therefore no per-request `CratestackContext` (``dlr.rs``'s own module doc).
...
. Read the alert's own `entity`/`from_state`/`to_state` labels -- they name
...
. Grep both `sms-gateway` and `sms-worker` logs for `cratestack_error` /
`+illegal ... transition ... on <id>+` around the alert's firing time --
the specific row id is in the trigger's own message.
. This is a code bug, not an operational incident to route around: some
...
(every process configured for the role is reporting "standing by") _or_
`+absent(...)+` (no process anywhere is even attempting the role).

. Check every `sms-worker` process's own `--roles`/`SMS_WORKER_ROLES` --
...
. If the role is configured somewhere, check that process's logs for
...
. `dispatch`/`drain` are `severity: critical` -- no messages submit, or
...
. Check for a second `sms-worker` process running `--roles` including
...
. Check Postgres's own `pg_locks` for the `dispatch` advisory lock
...
. Every submission this deployment makes carries no idempotency key to
...
. If `SMSDrainSingletonUnheld` is also firing, that's the root cause --
fix that first.
. If `drain` is held but this still fires, check `sms-worker` logs for
...
being held.
. Otherwise: a subscriber is failing repeatedly on the same event
...
. Read the per-row `warn!` logs ``reap_outbox``'s own `alert_poison_rows`
emits every run (hourly, §7.5's cadence) -- `model`/`operation`/
`last_error` name the actual subscriber bug.
. The row is never deleted automatically (``reap_outbox.rs``'s own module
...
deletion.
. If the fix genuinely requires discarding the event (it will never
...
[#route-divergence]
=== Route delivery-rate divergence
...
. This is a proxy signal, not a confirmed grey route -- §6.4's own
...
. Read the per-pair `warn!` log ``grey_route_watch``'s own `check_divergence`
...
. This metric knows nothing about whether the route is actually healthy --
...
. Run `docs/runbooks/grey-route-validation.adoc` against every route named
in the per-route `warn!` log (`route_id`/`route_name`/`last_validated`).
. A permanently retired route that will never carry traffic again should
