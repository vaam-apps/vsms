...
**`msisdn` is `@pii`** and `msisdnHash` is HMAC-SHA256 of the E.164 form under a server-held pepper. Analytics, dedupe, rate limiting and opt-out matching all go through the hash. Under Law No. 2024/017 that's the difference between a purge being one `UPDATE` and a forensic exercise.
...
Four limiters, easy to conflate:
