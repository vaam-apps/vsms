...
-- caller is `requeueJob` (backends/crates/sms-api/src/procedures.rs) — an operator's
-- explicit "try this again" action from the admin Jobs screen, never
-- proposed by the automatic pipeline (`backends/crates/sms-worker/src/jobs.rs`'s own
...
-- `replayWebhookAttempt` (backends/crates/sms-api/src/procedures.rs) is the only
...
`require_permission` (`backends/crates/sms-api/src/rbac.rs`) checks the literal
...
Steps 1 to 9 are all pre-persistence. A message that reaches the database is one you have already decided you can send — the worker's job is delivery, not validation. This diagram predates #72 and is not redrawn for it (mermaid parsing is CI-gated per this file's own note on that job; a diagram edit means re-verifying it, and this one is otherwise still accurate) — read `backends/crates/sms-api/src/consent.rs`'s own module doc for the real, current step order: two more accept-time checks now sit between "opt-out check" and "quota", both class-gated (`marketing`/`notification` only; `otp`/`transactional` are exempt) — self-imposed marketing quiet hours (08:00–20:00 WAT), then a `ConsentRecord` lookup.
...
**`SqlxIdempotencyStore::ensure_schema()` is never called by `sms-gateway` itself.** The `cratestack_idempotency` table it writes is library bookkeeping, not part of `schemas/vsms.cstack`, but the deploy path's own rule against runtime DDL applies to it anyway: `backends/migrations/postgres/0003_idempotency_table/up.sql` is the one committed, drift-tested copy of `IDEMPOTENCY_TABLE_DDL`'s exact text — living in the same directory, same directory-per-migration shape as `0001_init`/`0002_bootstrap` even though `cratestack migrate diff` never generated it — and every path that needs the table applies that same file rather than each keeping its own copy: `backends/apps/sms-migrate`'s advisory-lock-guarded one-shot migrate job (discovers and embeds every `backends/migrations/postgres/*/up.sql` via a build script and applies each with `cratestack::sqlx::raw_sql`, tracked in that binary's own `schema_migrations` bookkeeping table under the migration's own directory name, `0003_idempotency_table`, so the serving process needs no `CREATE TABLE` privilege and two replicas starting together never race a DDL statement), `ci/apply-migrations.sh` (the CI/scratch-database path), and `backends/crates/sms-test-support` (the live-suite harness's own per-binary template database) — all three now discover it by the same directory walk, with no special case for this file. `backends/crates/sms-api/tests/idempotency_table_ddl_matches_cratestack.rs` asserts the copy hasn't drifted from the pinned library's constant.
...
**`MtnProvider`** — MTN's own platform (MADAPI), self-service, and the Swagger is real: downloaded 2026-09-17 and vendored at `backends/crates/sms-provider-mtn/mtn-sms-v3-swagger.yaml`. This paragraph previously said MTN "publishes no pricing, no sender-ID policy and **no DLR spec**" — the last of those was simply wrong, and cost this repo an invented placeholder contract that sat in the tree for a milestone. There is a full DLR spec. Pricing and sender-ID policy genuinely are still absent, and go-live is still a commercial timeline.

- OAuth2 `client_credentials` against `https://api.mtn.com/v1/oauth/access_token/accesstoken?grant_type=client_credentials`, scope `SEND-SMS`. Not a static API key.
- `POST https://api.mtn.com/v3/sms/messages/sms/outbound`. Success is **`200`**, not `201` — and a `200` is not enough on its own: the body carries a 4-character MADAPI `statusCode` where `'0000'` means success, so a `200` with any other code is an application-level failure.
- Request: `receiverAddress` is an **array** of E.164 strings, plus `message`, `serviceCode` (the approved short code) and `clientCorrelatorId` (**`maxLength: 36`** — a truncated correlator makes every DLR for that message unmatchable). `senderAddress` is optional and takes precedence over `serviceCode` when set.
- **Correlation is the mirror image of Orange's.** MTN's DLR echoes back `clientCorrelatorId` — the value *you* supplied — while `transactionId` is MADAPI's own tracing id. Orange does the opposite, reporting its own `resource_id` and accepting no caller token at all. This is exactly what `providerMessageRefAlt` is for, and the reason that column survived the Orange cleanup that stopped populating it.
- **DLRs do not arrive by default.** You must send `requestDeliveryReceipt: true` *and* register a `deliveryReportUrl` through `POST /messages/sms/subscription`. Unlike Orange's manual support-ticket whitelisting, that registration is an API call, so it can be automated.
- `deliveryStatus` is exactly eight values: `ACCEPTD`, `DELETED`, `DELIVERED`, `ENROUTE`, `UNKNOWN`, `EXPIRED`, `REJECTED`, `UNDELIVERED` — SMPP `message_state` names (`ACCEPTD` is the five-character truncation). Note `ACCEPTD` and `ENROUTE` are *progress, not verdicts*, and map to `DeliveryOutcome::InFlight`; treating them as `uncertain` costs the message its retry path (§7.4, and that variant's own doc).
- The DLR carries no delivering-network field, so `delivering_network` is always `None` here — §6.4's grey-route detection gets nothing from MTN DLRs.

**Caveat on provenance:** `developers.mtn.com`'s TLS certificate was expired when the Swagger was fetched (Let's Encrypt, `notAfter Sep 16 06:02:41 2026 GMT`), so verification was bypassed for that one download at the maintainer's direction, with the chain otherwise intact. Re-download over a valid certificate and diff against the vendored copy before trusting it for anything commercial.
...
**Implementation, #40 (`backends/crates/sms-worker/src/hooks.rs`), landed against the above:**
...
**Implementation, #43 (`replayWebhookAttempt`, `backends/crates/sms-api/src/procedures.rs`), landed against the above:**
...
- cratestack diff schema/schema.prev.cstack schemas/vsms.cstack
- cratestack generate-typescript --schema schemas/vsms.cstack --out frontends/packages/sms-client \
...
├── frontends/apps/admin/                # Next.js 15 App Router + shadcn/ui
