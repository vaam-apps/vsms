...
**Write access is a separate question this PR does not close.** `Provider.update` and `Route.create`/`update`/`delete` are untouched — still `hasRole('owner') || hasRole('admin')` (plus `operator` for `Provider.update`), no `auth().kind == "app"` clause at all. `GatewayAuth` (§4, unchanged since #97/#98) hardcodes `role: "app"` for every real token this deployment mints, so no token it can currently issue satisfies that policy regardless of scope — the identical structural gap `#59`'s own finding already named for every `@version`'d model's write path. The admin console's edit/create/delete forms are real, tested code (`packages/gateway/src/providers.ts`/`routes.ts`, wired through `#59`'s own `fetchWithEtag`/`updateWithIfMatch`) that will 403 against a real gateway until #194 (human login) lands — stated on screen, not hidden behind a flag, matching this doc's own "no dormant code" convention.
...
**Providers/Routes/Simulator landed in #54.** List/detail for both models is real over real HTTP; editing (`Provider.update`, `Route.create`/`update`/`delete`) is real, tested code that cannot succeed against a real gateway until #194 (human login) lands — see §6.3's own `#54` implementation note for the full mechanism and why.
