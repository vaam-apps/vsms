...
3. **M6's remaining compliance items** — and these are legal, not technical: ~~consent records (#72)~~, audit anchoring (#68). **#72 resolved 2026-08-12**: `ConsentRecord` (§2.7), the `marketing`/`notification`-only opt-out and consent gate, and self-imposed marketing quiet hours (08:00–20:00 WAT) all landed in `sendMessage`, `crates/sms-api/src/consent.rs`. Retention purge (#67) is done — decision #5 resolved 2026-08-11 (90-day minimisation, no split ledger) and `purge_retention` shipped in the same PR that recorded the resolution. Law No. 2024/017 sanctions run to 100,000,000 FCFA and criminal penalties; this is the phase where "we'll do it after launch" is the expensive answer. Audit anchoring (#68) is the one item left on this list.
