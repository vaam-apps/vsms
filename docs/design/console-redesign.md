...
9. `@uidotdev/usehooks` for hook needs.
...
| D10 | English-only: **`labelFr`/`tooltipFr` data stays in `status-tokens.ts`** (cheap, may still serve a future non-UI consumer such as a CSV export or a compliance report — Cameroon is a bilingual administrative context per `docs/architecture.md`), but the `locale` **prop is removed** from every UI component's public API (`StatusPill`, `StateTimeline`) and every call site renders English only, unconditionally | Delete `labelFr`/`tooltipFr` entirely, since the UI constraint is English-only | The constraint is about the *console's UI*, not about deleting bilingual data that may be genuinely load-bearing elsewhere (this needs confirming with the maintainer — flagged, not assumed, in §7) | Constraint 1 |
...
| `bespoke/live-row.tsx` | Hand, composes `TableRow`, `matchMedia` reduced-motion check | Refresh (mechanism kept) | Reduced-motion check moves to `usehooks`' `useMediaQuery` (D12); wash behavior, timing, and the "in-place, never reflow" contract are unchanged — this encodes a real correctness property (§6.5 rule 3 in the existing design doc lineage), not chrome |
...
package. `cva` and `@uidotdev/usehooks` are added. `clsx`, `tailwind-merge`,
  ...
  return (
    <DrawerPrimitive.Portal>
      ...
      <DrawerPrimitive.Content
        className={cn(
          isMobile
            ? "fixed inset-x-0 bottom-0 z-50 max-h-[70vh] rounded-t-box border-edge border-t"
            : "fixed inset-y-0 right-0 z-50 h-full w-full max-w-[440px] border-edge border-l",
...
- **D10's `locale` removal touches four components' public API**
  ...
  (French labels/tooltips would keep compiling as unused dead code otherwise).
  **Also flag to the maintainer directly**: is `labelFr`/`tooltipFr` *data*
  (kept per D10) actually still wanted anywhere, or should it be deleted too?
  This document assumes "keep the data, drop the switch" but that is this
  document's own inference, not a stated constraint.
