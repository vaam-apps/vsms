...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  // `admin/middleware.ts` forwards the signed-in human's email as
  // `x-vsms-actor` on every authenticated request — the same header
  // `packages/api/src/context.ts` already reads for the tRPC `actor`
  // field. Reading it here, rather than importing `admin/lib/session.ts`
