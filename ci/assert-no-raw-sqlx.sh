...
hits=$(grep -rn --include='*.rs' -E 'sqlx::(query|query_as|query_scalar|raw_sql)\b' "${roots[@]}" \
       | grep -vE 'sms-worker/src/(lease|notify|drain|jobs/reap_outbox)\.rs|sms-api/src/cache\.rs|sms-test-support/src/lib\.rs|sms-gateway/src/health\.rs|sms-migrate/src/main\.rs' || true)
