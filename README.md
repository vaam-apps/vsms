...
| `schemas/vsms.cstack` | 22 models, 16 enums, 13 procedures. Parses, emits Postgres DDL, and expands through `include_server_schema!`. |
...
- **Rust** for the gateway and worker, on [CrateStack](https://cratestack.dev/) `=0.7.16` — schema-first, `.cstack` generates the model layer, policies, audit, events and REST surface. Pinned exactly, and the installed CLI must match the pin; see `AGENTS.md`.
