...
`packages/sms-client` is generated by `cratestack generate-typescript` at
CI/build time and is **not committed** (`packages/sms-client/GENERATING.md`).
Depending on it here would mean this example only builds inside a full
`vsms` checkout with `just client-gen` already run — the opposite of
"copy this into your own repo and it works." A third-party integrator
doesn't have this monorepo, `cratestack` on `PATH`, or the schema. So this
example depends on nothing but `jose` (RS256 signing — the same library
`packages/gateway/src/token.ts` already uses) and the platform `fetch`.
The tradeoff is real and worth stating: no generated types, no compile-time
